...
VOLUME_FILTER_DEV="volume=analyze-data_anl-riffdata-db-data"
MONGO_CONTAINER=$(docker ps --filter=$VOLUME_FILTER_DEV --filter="status=running" --format={{.Names}})
DATABASE_NAME="riff-test"
ARCHIVE_PATH=./riffdata
ARCHIVE_NAME=$1
